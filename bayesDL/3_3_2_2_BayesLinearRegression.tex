...
本では、積分を行い、(3.74)を求めることになっているが、面倒なので、(A.24)を利用することで(3.76),(3.77)を求める。(3.74)を求めるには(A.24)の導出を参考にすれば良い。
...
(A.1)で$A=\sigma_y^2, U = {\phi(x_*)}^T, B = \hat{\Sigma}, V = \phi(x_*)$とする。(A.2)で$P={\sigma_y}^{-2}, B = \phi(x_*), R = {\hat{\Sigma}}^{-1}$として、左辺の一番右の$R^{-1}$を右辺に移して、左辺を利用する。
