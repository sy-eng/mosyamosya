...
=\int p({\bf z}_{n-1} | {\bf x}_{n-1}) p({\bf z}_n | {\bf z}_{n - 1}, {\bf x}_{n-1}) d{\bf z}_{n-1}
=\int \hat{\alpha}({\bf z}_{n-1}) p({\bf z}_n | {\bf z}_{n - 1}) d{\bf z}_{n-1}
\end{split}
\end{equation}

まず、(13.87)を考える。(\ref{13_86})にて、すべて、${\bf x}_{n-1}$が条件づけされているので、{\bf ひとまず}、ないものとして考える。
...
= {\bf K}_n{\bf x}_n + ({\bf I} + {\bf A\mu}_{n-1} - {\bf K}_n{\bf C}){\bf A\mu}_{n-1}
